module Day14.Day14 where
    ...
    rollRocks :: Grid Char -> (Grid Char -> Coord -> Coord) -> (Grid Char -> [Coord]) -> Grid Char
    rollRocks grid getSpaceFn findFn = if not (length (findFn grid) > 0)
        then grid
        else rollRocks gridAfterRolling getSpaceFn findFn where
            gridAfterRolling = foldl (\a r -> swapGridCells a r (getSpaceFn grid r)) grid (findFn grid)

    findRocks grid = filter (\c -> getGridCell grid c == 'O') cells where
